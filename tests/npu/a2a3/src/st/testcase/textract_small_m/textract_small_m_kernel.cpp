    ...
    __gm__ float* out, __gm__ uint8_t* a, __gm__ uint8_t* b, uint32_t m, uint32_t k)
{
    ...
    constexpr int leftCols = C::Compact && C::M < 16 ? 2 * C::Cols : C::Cols;
    ...
    using Left = Tile<
        TileType::Left, T, 16, leftCols, BLayout::RowMajor, validM, validK, SLayout::RowMajor, 512, PadValue::Null,
        C::Compact ? CompactMode::Normal : CompactMode::Null>;
    using Right = TileRight<T, C::Cols, n, C::K, n>;
    using Acc = TileAcc<float, 16, n, C::M, n>;
    using GA = GlobalTensor<T, Shape<1, 1, 1, C::Rows, srcCols>, Stride<1, 1, 1, srcCols, 1>>;
    using GB = GlobalTensor<T, Shape<1, 1, 1, C::Cols, n>, Stride<1, 1, 1, n, 1>>;
    using GC = GlobalTensor<float, Shape<1, 1, 1, C::M, n>, Stride<1, 1, 1, n, 1>>;
    MatA matA;
    MatB matB;
    Left left;
    Right right;
