...
void LaunchTMins(T* out, T* src0, T* scalar, void* stream)
{
    if constexpr (std::is_same_v<T, aclFloat16>) {
        runTMins<half, dstTileH, dstTileW, src0TileH, src0TileW, vRows, vCols, padValueType>
            <<<1, nullptr, stream>>>((half*)(out), (half*)(src0), (half*)(scalar));
    } else {
        runTMins<T, dstTileH, dstTileW, src0TileH, src0TileW, vRows, vCols, padValueType>
            <<<1, nullptr, stream>>>(out, src0, scalar);
    }
}

template void LaunchTMins<float, 64, 64, 64, 64, 64, 64, PAD_VALUE_NULL>(
    float* out, float* src0, float* scalar, void* stream);
template void LaunchTMins<int32_t, 64, 64, 64, 64, 64, 64, PAD_VALUE_NULL>(
    int32_t* out, int32_t* src0, int32_t* scalar, void* stream);
template void LaunchTMins<aclFloat16, 64, 64, 64, 64, 64, 64, PAD_VALUE_NULL>(
    aclFloat16* out, aclFloat16* src0, aclFloat16* scalar, void* stream);
template void LaunchTMins<int16_t, 64, 64, 64, 64, 64, 64, PAD_VALUE_NULL>(
    int16_t* out, int16_t* src0, int16_t* scalar, void* stream);
template void LaunchTMins<float, 60, 128, 64, 64, 60, 60, PAD_VALUE_MAX>(
    float* out, float* src0, float* scalar, void* stream);
template void LaunchTMins<int32_t, 60, 128, 64, 64, 60, 60, PAD_VALUE_MAX>(
    int32_t* out, int32_t* src0, int32_t* scalar, void* stream);
template void LaunchTMins<aclFloat16, 1, 3600, 2, 4096, 1, 3600, PAD_VALUE_MAX>(
    aclFloat16* out, aclFloat16* src0, aclFloat16* scalar, void* stream);
template void LaunchTMins<int16_t, 16, 256, 20, 512, 16, 200, PAD_VALUE_MAX>(
    int16_t* out, int16_t* src0, int16_t* scalar, void* stream);
