...
#define INIT_STACK 8192
    ...
    int dstOffset = (block_idx) * (shape3 / block_num) * shape4;
    auto dstGlobal =
        getGlobalTensor<T, shape0, shape1, shape2, shape3, shape4, kGTRows, shape4, BLayout::RowMajor, dyn_>(
            out + dstOffset, gShape0, gShape1, gShape2, kGTRows, shape4);

    // Load once into UB so timed TSTORE has real payload.
    ...
    for (int i = 0; i < 5; ++i) {
        if constexpr (useNotAlloc) {
            TSTORE<TStoreL2Hint::NotAllocClean>(dstGlobal, vecTile);
        } else {
            TSTORE<TStoreL2Hint::NormalFirstVictim>(dstGlobal, vecTile);
        }
    }
    t1 = get_syscnt();
#ifndef __PTO_AUTO__
...
extern "C" __global__ AICORE void launchTSTORE_1(
    __gm__ uint8_t* out, __gm__ uint8_t* src, int gShape0, int gShape1, int gShape2, int gRows, int gCols,
    __gm__ uint64_t* gLog)
{
    // A5_TSTORE_VEC_ALLOC: float 128x256, NormalFirstVictim, TSTORE x5
    runTSTOREND<float, 1, 1, 1, 128, 256, 128, 256, 1, false, PadValue::Null>(
        (__gm__ float*)out, (__gm__ float*)src, gShape0, gShape1, gShape2, gRows, gCols, gLog);
}

extern "C" __global__ AICORE void launchTSTORE_2(
    __gm__ uint8_t* out, __gm__ uint8_t* src, int gShape0, int gShape1, int gShape2, int gRows, int gCols,
    __gm__ uint64_t* gLog)
{
    // A5_TSTORE_VEC_NOTALLOC: float 128x256, NotAllocClean, TSTORE x5
    runTSTOREND<float, 1, 1, 1, 128, 256, 128, 256, 1, true, PadValue::Null>(
        (__gm__ float*)out, (__gm__ float*)src, gShape0, gShape1, gShape2, gRows, gCols, gLog);
}

template <int32_t testKey>
void launchTSTORE(uint8_t* out, uint8_t* src, uint64_t* gLog, void* stream)
{
    if constexpr (testKey == 1) {
        launchTSTORE_1<<<1, nullptr, stream>>>(out, src, 1, 1, 1, 128, 256, gLog);
    } else if constexpr (testKey == 2) {
        launchTSTORE_2<<<1, nullptr, stream>>>(out, src, 1, 1, 1, 128, 256, gLog);
    }
}
...
int get_input_golden_case(uint8_t* input, uint8_t* golden)
{
    ...
    T gold_arr[Shape0][Shape1][Shape2][Shape3][Shape4] = {};
                ...
                for (int i = 0; i < Shape3; i++) {
                    for (int j = 0; j < Shape4; j++) {
                        in_arr[x0][x1][x2][i][j] = x0 * Shape1 * Shape2 * Shape3 * Shape4 +
                                                   x1 * Shape2 * Shape3 * Shape4 + x2 * Shape3 * Shape4 + i * Shape4 +
                                                   j;
    ...
    std::copy((uint8_t*)gold_arr, ((uint8_t*)(gold_arr)) + in_byteSize, golden);
...
int get_input_golden(uint8_t* input, uint8_t* golden)
{
    if constexpr (testKey == 1 || testKey == 2) {
        return get_input_golden_case<float, 1, 1, 1, 128, 256>(input, golden);
    }
    return 0;
}

template void launchTSTORE<1>(uint8_t* out, uint8_t* src, uint64_t* gLog, void* stream);
template void launchTSTORE<2>(uint8_t* out, uint8_t* src, uint64_t* gLog, void* stream);
