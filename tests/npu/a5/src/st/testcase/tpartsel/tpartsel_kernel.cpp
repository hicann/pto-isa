    ...
    __gm__ T __out__* out, __gm__ uint8_t __in__* mask, __gm__ T __in__* src0, __gm__ T __in__* src1)
{
    ...
    Event<Op::TLOAD, Op::TPARTSEL> loadEvent;
    Event<Op::TPARTSEL, Op::TSTORE_VEC> storeEvent;
    TLOAD(src0Tile, src0Global);
    TLOAD(src1Tile, src1Global);
    loadEvent = TLOAD(maskTile, maskGlobal);
    storeEvent = TPARTSEL(dstTile, maskTile, src0Tile, src1Tile, tmpTile, loadEvent);
    TSTORE(dstGlobal, dstTile, storeEvent);
}
...
template void LaunchTPartSel<int8_t, 4, 320, 3, 259>(
    int8_t* out, uint8_t* mask, int8_t* src0, int8_t* src1, void* stream);
template void LaunchTPartSel<uint8_t, 4, 320, 3, 259>(
    uint8_t* out, uint8_t* mask, uint8_t* src0, uint8_t* src1, void* stream);
template void LaunchTPartSel<int64_t, 4, 128, 3, 67>(
    int64_t* out, uint8_t* mask, int64_t* src0, int64_t* src1, void* stream);
template void LaunchTPartSel<uint64_t, 4, 128, 3, 67>(
