...
PTO_INTERNAL void TInterleaveStoreSplit(RegTensor<T> &dst0Reg, RegTensor<T> &dst1Reg, __ubuf__ T *dstPtr,
                                        uint32_t dstOffset, DistValue distValue, uint32_t lowerMask, uint32_t upperMask)
{
...
template <typename T, unsigned ElementsPerRepeat, typename DistValue>
PTO_INTERNAL void TInterleaveAlignedHalf(RegTensor<T> &src0Reg, RegTensor<T> &src1Reg, RegTensor<T> &dst0Reg,
                                         RegTensor<T> &dst1Reg, __ubuf__ T *dstPtr, __ubuf__ T *src0Ptr,
                                         __ubuf__ T *src1Ptr, uint32_t srcBase, uint32_t dstBase, uint16_t repeatTime,
                                         uint32_t validCols, DistValue distValue)
{
    ...
    for (uint16_t j = 0; j < repeatTime; ++j) {
        ...
        TInterleaveStoreSplit<T, ElementsPerRepeat>(dst0Reg, dst1Reg, dstPtr, dstBase + j * 2 * ElementsPerRepeat,
                                                    distValue, lowerMask, upperMask);
    }
}

template <typename T, unsigned ElementsPerRepeat, typename DistValue>
PTO_INTERNAL void TInterleaveUnalignedHalf(RegTensor<T> &src0Reg, RegTensor<T> &src1Reg, RegTensor<T> &dst0Reg,
                                           RegTensor<T> &dst1Reg, UnalignReg &ureg, __ubuf__ T *dstPtr,
                                           __ubuf__ T *src0Ptr, __ubuf__ T *src1Ptr, uint32_t srcBase, uint32_t dstBase,
                                          ...
                                          unsigned validCols)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < (uint16_t)validRows; ++i) {
            ...
            TInterleaveAlignedHalf<T, ElementsPerRepeat>(src0Reg, src1Reg, dst0Reg, dst1Reg, dst0Ptr, src0Ptr, src1Ptr,
                                                         i * srcRowStride, i * dstRowStride, repeatTime, validCols,
                                                         distValue);
            TInterleaveAlignedHalf<T, ElementsPerRepeat>(src0Reg, src1Reg, dst0Reg, dst1Reg, dst1Ptr, src0Ptr, src1Ptr,
                                                         i * srcRowStride + halfValidCols, i * dstRowStride, repeatTime,
                                                         validCols, distValue);
        }
    }
}
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < (uint16_t)validRows; ++i) {
            ...
            TInterleaveAlignedHalf<T, ElementsPerRepeat>(src0Reg, src1Reg, dst0Reg, dst1Reg, dst0Ptr, src0Ptr, src1Ptr,
                                                         i * srcRowStride, i * dstRowStride, repeatTime, validCols,
                                                         distValue);
            TInterleaveUnalignedHalf<T, ElementsPerRepeat>(src0Reg, src1Reg, dst0Reg, dst1Reg, ureg, dst1Ptr, src0Ptr,
                                                           src1Ptr, i * srcRowStride + halfValidCols, i * dstRowStride,
