...
<!-- md-trans-meta sourceCommit=unknown translatedAt=2026-08-26T04:57:10.118Z pushedAt=2026-08-29T09:05:18.462Z -->
...
Row broadcast maximum: takes the maximum of each row of the full-size operand (`src0` or `src1`) and the scalar per row of the expanded operand.
...
Assume that `b_i` is the 32-byte block obtained from the expanded operand for row `i` (RowMajor layout, `32 / sizeof(T)` values per row). The block repeats naturally within each vector repeat stride.
...
- **Atlas A2/A3 training series products/Atlas A2/A3 inference series products**: The tmp tile is used as a broadcast buffer. The scalar value per row of the ColMajor expanded operand is broadcast to the tmp buffer through the `vbrcb` instruction, creating a 32-byte block for each row, which is then used as the expanded operand in the binary operation. The repeat stride of the `vbrcb` instruction is 8 blocks (256 bytes), and each repeat processes 8 rows. The minimum tmp size is calculated as follows:
    - **Common parameters**:
        - `R = dst.GetValidRow()`, `T = TileDataDst::DType`.
    - When `R < 256`:
        $$ \text{tmpSize} = \left\lceil\frac{R}{8}\right\rceil \times 256 \text{ bytes} $$
    - When `R >= 256`:
        - The operation uses a loop, with up to 30 repeats (240 rows) per loop. The tmp buffer is reused across iterations, and each iteration requires:
...
- **Ascend 950PR/Ascend 950DT**: The `tmp` tile is accepted but not used (`[[maybe_unused]]`). Ascend 950PR/Ascend 950DT hardware natively supports row broadcast through the broadcast mode of the `vlds` instruction, so no temporary buffer is required.
