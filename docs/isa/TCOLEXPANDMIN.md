...
<!-- md-trans-meta sourceCommit=unknown translatedAt=2026-08-26T03:46:36.373Z pushedAt=2026-08-29T09:05:18.422Z -->
...
Column broadcast minimum: takes the minimum with the per-column scalar vector.
...
- `TileDataDst::DType` and `TileDataSrc1::DType` must be one of the following: `half`, `float`, `int16`; `int32` applies to Atlas A2 training products/Atlas A2 inference products, Atlas A3 training products/Atlas A3 inference products, and Ascend 950PR/Ascend 950DT; `uint16`, `uint32`, `bfloat16_t`, `int8`, and `uint8` apply to Ascend 950PR/Ascend 950DT.
- Tile shape/layout constraints (compile time): `TileDataDst::isRowMajor`.
- `src1` is expected to provide **one scalar per column** (that is, its valid shape must cover `C` values).
