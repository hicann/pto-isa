...
<!-- md-trans-meta sourceCommit=unknown translatedAt=2026-08-26T05:12:52.878Z pushedAt=2026-08-29T09:05:18.469Z -->
...
Performs element-wise left shift on two tiles.
...
- **Implementation check (Ascend 950PR/Ascend 950DT)**:
    - Supported element types are `uint8_t`, `int8_t`, `uint16_t`, `int16_t`, `uint32_t`, and `int32_t` (consistent with Atlas A2/A3 training products/Atlas A2/A3 inference products).
    - `dst`, `src0`, and `src1` must use the same element type.
    - `dst`, `src0`, and `src1` must be row-major.
    - Runtime: `src0.GetValidRow()/GetValidCol()` and `src1.GetValidRow()/GetValidCol()` must be consistent with `dst`.
- **Valid region**:
