...
- **UB placement:** `tmp` should be placed right after `dst` (32-B aligned), sized `ceil(C·b, 32)` bytes (equivalently `ceil(ceil(C, 32)·b, 32)` since $b \in \{2,4\}$ divides 32) — not at a fixed 8 KB offset, since Path A needs up to ~32 KB for float near the threshold.
